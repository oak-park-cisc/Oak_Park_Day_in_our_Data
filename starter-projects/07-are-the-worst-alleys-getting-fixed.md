...
- Cached in this repo: `data/alleys-oak-park.csv` (both layers already joined, one row per segment with `pci_2022_2023`, `pci_2024`, `pci_change`, `scheduled_build_year`, `in_2026_cip`, and a centroid; 640 rows)
- Cached in this repo: `data/alleys-oak-park.geojson` (the same rows with the line geometry)
- Cached in this repo: `data/capital-projects-oak-park.geojson` (all 12 2026 CIP layers in one file with a `project_type` column, including the alley reconstruction plan)
- Cached in this repo: `data/streets-oak-park.geojson` (Village centerlines with address ranges)
